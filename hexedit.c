...
void RenderFrame(HWND hwnd, HDC hdc) {
    ...
    HBRUSH menuBrush = CreateSolidBrush(cfg.menu_bg);
    FillRect(memDC, &topRect, menuBrush);
            ...
            for (int j = 0; j < bpr; j++) {
                if (offset + j < editor.file_size) {
                    ...
                    if (is_sel) { 
                        ...
                        FillRect(memDC, &r, hSelBrush); 
                    }
                    if (is_cursor) { 
                        RECT r = {xHex + j*3*charWidth, y, xHex + (j+1)*3*charWidth, y + charHeight}; 
                        FillRect(memDC, &r, hCurBrush); 
    ...
    HBRUSH statusBrush = CreateSolidBrush(cfg.status_bg);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CREATE:
            ...
            hFont = CreateFontA(cfg.font_size, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET, 
        ...
        case WM_SIZE:
            if (!is_snapping) user_resized = TRUE; 
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    wc.hbrBackground = CreateSolidBrush(cfg.bg_color); 
