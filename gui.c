...
static void SnapWindowSize(HWND hwnd, int rows)
{
    ...
    int win_w = (int)((double)win_h * (double)cfg.aspect_w /
                      (double)cfg.aspect_h);
...
static void UpdateVScroll(HWND hwnd)
{
    ...
    size_t maxScrollRow =
        (totalRows > (size_t)pageRows) ? (totalRows - (size_t)pageRows) : 0;
...
static void HandleVScroll(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (LOWORD(wParam)) {
        ...
        case SB_THUMBPOSITION: {
            ...
            if (GetScrollInfo(hsb, SB_CTL, &ti))
                pos = ti.nTrackPos;
...
static void OnSize(HWND hwnd, int cx, int cy)
{
    ...
    if (charHeight > 0) {
        visibleRows =
            (clientRect.bottom - MENU_HEIGHT - STATUS_HEIGHT) / charHeight;
...
static void EnsureCursorVisible(void)
{
    ...
    if (cursor_row < view_row) {
        ...
        editor.view_offset =
            (cursor_row - (size_t)visibleRows + 1) * (size_t)bpr;
    }
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        {
            RECT rcNow;
            if (GetClientRect(hwnd, &rcNow))
                OnSize(hwnd, rcNow.right, rcNow.bottom);
        }
    ...
    case WM_VSCROLL:
        HandleVScroll(hwnd, wParam, lParam);
        break;
    ...
    case WM_PAINT: {
        ...
        if (paintRect.right <= 0 || paintRect.bottom <= 0)
        ...
        TextOutA(memDC, 10, paintRect.bottom - 20,
        ...
        BitBlt(hdc, 0, 0, paintRect.right, paintRect.bottom,
               memDC, 0, 0, SRCCOPY);
    ...
    case WM_LBUTTONDOWN: {
        ...
        if (off != (size_t)-1) {
            ...
            if (GetKeyState(VK_SHIFT) & 0x8000) {
                if (editor.selection_start == (size_t)-1)
    ...
    case WM_KEYDOWN: {
        ...
        switch (wParam) {
            ...
            case VK_DOWN:
                if (can_extend && editor.memory_mode) {
                    editor.cursor += (size_t)bpr;
                } else {
                    if (editor.cursor + (size_t)bpr < vs)
                        editor.cursor += (size_t)bpr;
                }
                break;
            ...
            case VK_RIGHT:
                if (can_extend && editor.memory_mode) {
                    ...
                    if (vs > 0 && editor.cursor < vs - 1)
                        editor.cursor++;
                }
                break;

            case VK_PRIOR:
                if (editor.cursor >= page)
                    editor.cursor -= page;
                else
                    editor.cursor = 0;
                break;
        ...
        if (shift) {
            editor.selection_end = editor.cursor;
        } else if (editor.cursor != old_cursor) {
            clear_selection(&editor);
        }

        if (editor.cursor != old_cursor)
            hex_state = 0;
        ...
        if (editor.edit_mode == 0) {
            ...
            if (val != -1) {
                if (hex_state == 0) {
                    ...
                    if (editor.memory_mode)
                        editor.cursor++;
                    else if (editor.cursor < new_size)
                        editor.cursor++;
